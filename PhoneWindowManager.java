...
public class PhoneWindowManager implements WindowManagerPolicy {
	...
	//add by hj 
    private boolean mF1KeyTriggered;
    private boolean mF2KeyTriggered;
	private long mF1KeyTime;
	private long mF2KeyTime;
	private boolean mF1KeyConsumedByAPPshotChord;
	private boolean mF2KeyConsumedByAPPshotChord;
    ...
    public void setInitialDisplaySize(Display display, int width, int height, int density) {
        ...
        //mNavigationBarCanMove = width != height && shortSizeDp < 600;  /*Modify by Tony*/
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
		...
		//add by hj
		if (mF1KeyTriggered || mF2KeyTriggered){

			final long now = SystemClock.uptimeMillis();
			final long timeoutTime = mF2KeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS;  
			if (now < timeoutTime) {
				return timeoutTime - now;
			}
		}
		//add by hj
		if (keyCode == KeyEvent.KEYCODE_F2&& mF2KeyConsumedByAPPshotChord) {
			if (!down) {
				mF2KeyConsumedByAPPshotChord = false;
			}
			return -1;
		}
		//add by hj
		if (keyCode == KeyEvent.KEYCODE_F1 && mF1KeyConsumedByAPPshotChord) {
			if (!down) {
				mF1KeyConsumedByAPPshotChord = false;
			}
			return -1;
		}
	...
	private void interceptQuickMemoChord() {
		...
		if ( mF1KeyTriggered && mF2KeyTriggered ){
			final long now = SystemClock.uptimeMillis();
			try { 
				mContext.getPackageManager().getPackageInfo("com.microchip.calibration",0);
			} catch (PackageManager.NameNotFoundException e) {
				Log.e(TAG, "com.microchip.calibration");
				return;
			}
			
			if (now <= mF1KeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS &&
					now <= mF2KeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS) {
				mF1KeyConsumedByAPPshotChord = true;
				mF2KeyConsumedByAPPshotChord = true;
	...
	private final Runnable mQuickMemoChordPress = new Runnable() {
		...
		//intent.setClassName("com.aplex.webcan", "com.aplex.webcan.activity.Calibration");
		intent.setClassName("com.microchip.calibration", "com.microchip.calibration.Calibration");
        ...
        switch (keyCode) {
			...
			case KeyEvent.KEYCODE_F2: {   
					...
					if (keyCode == KeyEvent.KEYCODE_F1){
						if (down) {
	                        if ((event.getFlags() & KeyEvent.FLAG_FALLBACK) == 0) {
								mF1KeyTriggered = true;
								mF1KeyConsumedByAPPshotChord = false;
								mF1KeyTime = event.getDownTime();
	                            interceptQuickMemoChord();
	                        }
                    	}else{
							mF1KeyConsumedByAPPshotChord = false;
						...
						if (down) {
	                        if ((event.getFlags() & KeyEvent.FLAG_FALLBACK) == 0) {
								mF2KeyTriggered = true;
								mF2KeyConsumedByAPPshotChord = false;
								mF2KeyTime = event.getDownTime();
	                            interceptQuickMemoChord();
	                        }
                    	}else{
							mF2KeyConsumedByAPPshotChord = false;
